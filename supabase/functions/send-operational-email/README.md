...
Provider-neutral dispatcher for essential OutZila rental emails. The
...
- `EMAIL_FROM`: verified sender, for example `OutZila <operacoes@updates.outzila.com>`.
- `EMAIL_REPLY_TO`: optional monitored support address.
- `APP_BASE_URL`: public OutZila origin, without a trailing path.
...
3. Create an INSERT Database Webhook for `private.operational_email_outbox` that calls the function with the configured private dispatch-secret header.
...
- A sandbox sender is only temporary and
